...
# 출력 리다이렉션
ls -la > file_list.txt          # 파일에 저장
ls -la >> file_list.txt         # 파일에 추가

# 에러 리다이렉션
command 2> error.log            # 에러만 파일에
command > output.log 2>&1       # 출력과 에러를 모두 파일에
```

### 와일드카드 사용
```bash
ls *.txt                        # txt 확장자 파일
ls file?.txt                    # file1.txt, file2.txt 등
...
이 가이드는 리눅스 시스템 관리와 일상적인 작업에 필요한 핵심 명령어들을 포함하고 있습니다. 각 명령어의 기본 사용법부터 고급 옵션까지 실무에서 바로 활용할 수 있도록 구성했습니다.
